...
static unsigned char my_read_reg(struct oled_dev *dev, u8 reg)
{
	u8 data = 0;

	my_read_regs(dev, reg, &data, 1);
	return data;

}


static void my_write_reg(struct oled_dev *dev, u8 reg, u8 data)
{
	u8 buf = 0;
	buf = data;
	my_write_regs(dev, reg, &buf, 1);
}


void my_readdata(struct oled_dev *dev)
{
	// unsigned char i =0;
    // unsigned char buf[6];
...
void OLED_WR_Byte(struct oled_dev *dev,unsigned dat,unsigned cmd)
{
    // struct i2c_client *client= (struct i2c_client *)dev->private_data;
    // char *buf[3];
    int ret = 0;
	if(cmd == OLED_DATA)
	{
          my_write_reg(dev,0x78,0x40);
          my_write_reg(dev,0x78,dat); 
        //  buf[0] = 0x78;
        //   buf[1] = 0x40;
        //   buf[2] = dat;
...
{  
	...
	{  
		...
		OLED_WR_Byte (dev,0x00,OLED_CMD);      //设置显示位置—列低地址
		OLED_WR_Byte (dev,0x10,OLED_CMD);      //设置显示位置—列高地址   
		for(n=0;n<128;n++)OLED_WR_Byte(dev,1,OLED_DATA); 
	} //更新显示
}
...
static int oled_open(struct inode *inode, struct file *filp)
{
    ...
    // struct i2c_client *client= (struct i2c_client *)oleddev.private_data;
    char buf[3] = {0x01,0x02,0x03};

    // ret = i2c_master_send(client, buf,3);
    // if(ret < 0)
    // {
    //     printk("i2c_master_send error ret = %d",ret);
    // }
    //  my_write_reg (&oleddev,0x78,OLED_CMD);
     my_write_regs(&oleddev, 0x78, &buf, 3);
 #if 0
    OLED_WR_Byte(dev,0xAE,OLED_CMD);//--display off
	OLED_WR_Byte(dev,0x00,OLED_CMD);//---set low column address
	OLED_WR_Byte(dev,0x10,OLED_CMD);//---set high column address
	OLED_WR_Byte(dev,0x40,OLED_CMD);//--set start line address  
	OLED_WR_Byte(dev,0xB0,OLED_CMD);//--set page address
	OLED_WR_Byte(dev,0x81,OLED_CMD); // contract control
	OLED_WR_Byte(dev,0xFF,OLED_CMD);//--128   
	OLED_WR_Byte(dev,0xA1,OLED_CMD);//set segment remap 
	OLED_WR_Byte(dev,0xA6,OLED_CMD);//--normal / reverse
	OLED_WR_Byte(dev,0xA8,OLED_CMD);//--set multiplex ratio(1 to 64)
	OLED_WR_Byte(dev,0x3F,OLED_CMD);//--1/32 duty
	OLED_WR_Byte(dev,0xC8,OLED_CMD);//Com scan direction
	OLED_WR_Byte(dev,0xD3,OLED_CMD);//-set display offset
	OLED_WR_Byte(dev,0x00,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xD5,OLED_CMD);//set osc division
	OLED_WR_Byte(dev,0x80,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xD8,OLED_CMD);//set area color mode off
	OLED_WR_Byte(dev,0x05,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xD9,OLED_CMD);//Set Pre-Charge Period
	OLED_WR_Byte(dev,0xF1,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xDA,OLED_CMD);//set com pin configuartion
	OLED_WR_Byte(dev,0x12,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xDB,OLED_CMD);//set Vcomh
	OLED_WR_Byte(dev,0x30,OLED_CMD);//
	
	OLED_WR_Byte(dev,0x8D,OLED_CMD);//set charge pump enable
	OLED_WR_Byte(dev,0x14,OLED_CMD);//
	
	OLED_WR_Byte(dev,0xAF,OLED_CMD);//--turn on oled panel
#endif 
